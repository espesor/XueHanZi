...
- 状态：v0.1 已实现，字库尚未人工校对（见 §2.6）
- 最后更新：2026-07-30
- 仓库：[github.com/espesor/XueHanZi](https://github.com/espesor/XueHanZi)
...
| 颜色 | 含义 | 判定 |
|---|---|---|
| 🟢 **绿 · 已学会** | 稳了，靠抽查 / 按级复习巩固 | 连续两次不看解释就认识 |
...
**绿字会掉回橙色**。已学会的字如果复习时答错或需要查看，`streak` 和 `rep`（巩固次数）一起归零，重新来过。
...
- **字体**：汉字用**系统黑体**（`PingFang SC / HarmonyOS Sans SC / MiSans / Noto Sans CJK SC / Microsoft YaHei` …）——覆盖 iOS、华为、小米、其他安卓、Windows 各自的系统默认中文字体，也就是用户手机打字时天天看到的那一路字体，保证肯定命中。最初选的是楷体（字帖用字，笔画结构比黑体清楚），但 `Kaiti SC / STKaiti / KaiTi` 这些字体名基本只有 Apple / Windows 内置，大多数安卓手机没装，会一路 fallback 到通用 `serif`，渲染出来的衬线字很难看——这个风险在 §6.4 原文里判断为「可接受」，实测证明判断错了，改回系统黑体。界面文字用系统 UI 字体（同一路思路），数字用等宽字体（`tabular-nums`，避免跳动）。
...
> 注意：CJK 字体文件动辄几 MB，**不内嵌字体**，只用系统字体栈——这也是选系统黑体而不是楷体的另一半理由，字体名对不上时的 fallback 后果必须自己兜得住。
...
没有「下一个新字」指针字段——下一个该学的字直接扫一遍 `cards` 找第一个空缺下标现算（`nextNewIndex`）。不存指针是因为导入进度之后指针可能对不上，扫一遍最保险，3500 字的开销可以忽略。
...
│   ├── export_csv.py       导出全字库校对表
│   └── raw/                3500 字表 + CC-CEDICT（不进版本库，缺了自动下载）
├── scripts/
...
│   ├── main.ts             启动、加载字库、装配
│   ├── types.ts
│   ├── srs.ts              判定 + 复习队列（纯函数，有单测）
│   ├── progress.ts         等级、里程碑、按级分桶（纯函数，有单测）
│   ├── storage.ts          读写、导出、导入
│   ├── speech.ts           朗读
│   ├── styles.css
│   └── ui/                 learn / review / reviewPicker / sheet / celebrate / toast
└── test/
    ├── srs.test.ts
    └── progress.test.ts
```
...
核心规则（`srs.ts`、`progress.ts`）写成**纯函数**：输入卡片状态 + 答案 + 当前时间，输出新卡片状态。不碰 DOM，不碰 `Date.now()`（时间从参数传入）。这样单测能直接验证「连对两次转绿」「抽查封顶后橙色优先级还在」这类规则，不用起浏览器。
...
| 汉字字体 | 系统黑体（原定楷体） | 楷体字体名安卓大多没装，会 fallback 到丑的通用 serif；系统黑体保证所有手机都命中 |
